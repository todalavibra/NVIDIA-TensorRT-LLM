...
class GuidedDecodingParams:
    ...
    Args:
        json: The generated text is amenable to json format with additional user-specified restrictions, namely schema.
        regex): The generated text is amenable to the user-specified regular expression.
        grammar: The generated text is amenable to the user-specified extended Backus-Naur form (EBNF) grammar.
        json_object: If True, the generated text is amenable to json format.
        structural_tag: The generated text is amenable to the user-specified structural tag.
    """
...
class LogitsProcessor(ABC):
    ...
    def __call__(
        ...
        """Logits processing callback.

        The callback is expected to inplace modify the logits.

        Args:
            req_id (int): Request id.
            logits (torch.Tensor): Logits tensor to be modified.
            token_ids (List[List[int]]): Token ids produced by the request so far. The shape is
                beam_width * sequence_length.
...
class AdditionalModelOutput:
    ...
    Args:
        ...
        gather_context (bool): A value indicating whether or not to gather the additional output from the
            context too.
    """
...
class SamplingParams:
    ...
    Usage Examples:
        ...
        use_beam_search is False:
            ...
            - best_of is not None: (top-p/top-k) sampling best_of responses and return n generations
                (best_of >= n must hold)
        use_beam_search is True:
            - best_of is None: beam search with beam width of n, return n generations
            - best_of is not None: beam search with beam width of best_of, return n generations
                (best_of >= n must hold)

    Args:
        end_id: The end token id.
        pad_id: The pad token id.
        max_tokens: The maximum number of tokens to generate. Defaults to 32.
        bad: A string or a list of strings that redirect the generation
            when they are generated, so that the bad strings are excluded from the returned output.
        bad_token_ids: A list of token ids that redirect the generation when
            they are generated, so that the bad ids are excluded from the returned output.
        stop: A string or a list of strings that stop the generation when they
            are generated. The returned output will not contain the stop strings unless include_stop_str_in_output
            is True.
        stop_token_ids: A list of token ids that stop the generation when they are generated.
        include_stop_str_in_output: Whether to include the stop strings in output text.
        embedding_bias: The embedding bias tensor. Expected type is kFP32 and shape is [vocab_size].
        logits_processor: The logits postprocessor callback(s).
            If a list, each processor is applied in order during generation (supported in PyTorch backend only).
        apply_batched_logits_processor: Whether to apply batched logits postprocessor callback.
            The BatchedLogitsProcessor class is recommended for callback creation. The callback must
            be provided when initializing LLM.

        n: Number of sequences to generate. Defaults to 1.
        best_of: Number of sequences to consider for best output.
        use_beam_search: Whether to use beam search.

        top_k: Controls number of logits to sample from. None means using C++ runtime default 0, i.e., all logits.
        top_p: Controls the top-P probability to sample from. None means using C++ runtime default 0.f.
        top_p_min: Controls decay in the top-P algorithm. topPMin is lower-bound. None means using C++ runtime
            default 1.e-6.
        top_p_reset_ids: Controls decay in the top-P algorithm. Indicates where to reset the decay.
            None means using C++ runtime default 1.
        top_p_decay: Controls decay in the top-P algorithm. The decay value.
            None means using C++ runtime default 1.f.
        seed: Controls the random seed used by the random number generator in sampling.
            None means using C++ runtime default 0.
        temperature: Controls the modulation of logits when sampling new tokens. It can have values > 0.f.
            None means using C++ runtime default 1.0f.
        min_tokens: Lower bound on the number of tokens to generate. Values < 1 have no effect.
            None means using C++ runtime default 1.
        beam_search_diversity_rate: Used to penalize tokens based on how often they appear in the sequence.
            It can have any value > 0.f. Values < 1.f encourages repetition, values > 1.f discourages it.
            None means using C++ runtime default 1.f.
        repetition_penalty: Used to penalize tokens based on how often they appear in the sequence.
            It can have any value > 0.f. Values < 1.f encourages repetition, values > 1.f discourages it.
            None means using C++ runtime default 1.f.
        presence_penalty: Used to penalize tokens already present in the sequence
            (irrespective of the number of appearances). It can have any values. Values < 0.f encourage repetition,
            values > 0.f discourage it. None means using C++ runtime default 0.f.
        frequency_penalty: Used to penalize tokens already present in the sequence
            (dependent on the number of appearances). It can have any values. Values < 0.f encourage repetition,
            values > 0.f discourage it. None means using C++ runtime default 0.f.
        length_penalty: Controls how to penalize longer sequences in beam search.
            None means using C++ runtime default 0.f.
        early_stopping: Controls whether the generation process finishes once beamWidth sentences are generated
            (ends with end_token).  None means using C++ runtime default 1.
        no_repeat_ngram_size: Controls how many repeat ngram size are acceptable.
            None means using C++ runtime default 1 << 30.
        min_p: scale the most likely token to determine the minimum token probability.
            None means using C++ runtime default 0.0.
        beam_width_array: The array of beam width using in Variable-Beam-Width-Search.

        logprobs: Number of log probabilities to return per output token.
        prompt_logprobs: Number of log probabilities to return per prompt token.
        return_context_logits: Controls if Result should contain the context logits.
        return_generation_logits: Controls if Result should contain the generation logits.
        exclude_input_from_output: Controls if output tokens in Result should include the input tokens.
        return_encoder_output: Controls if Result should contain encoder output hidden states
            (for encoder-only and encoder-decoder models).
        return_perf_metrics: Controls if Result should contain the performance metrics for this request.
        additional_model_outputs: The additional outputs to gather from the model.

        lookahead_config: Lookahead decoding config.
        guided_decoding: Guided decoding params.
        ...
        add_special_tokens: Whether to add special tokens to the prompt.
        truncate_prompt_tokens: If set to an integer k, will use only the last k tokens from the prompt
            (i.e., left truncation).
