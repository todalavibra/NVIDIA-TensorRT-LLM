...
class TorchLlmArgs(BaseLlmArgs):
    ...
    def validate_cuda_graph_config(self) -> 'TorchLlmArgs':
        ...
        if self.cuda_graph_config is not None:
            if self.cuda_graph_config.cuda_graph_batch_sizes:
                self.cuda_graph_config.cuda_graph_batch_sizes = sorted(
                    self.cuda_graph_config.cuda_graph_batch_sizes)
                if self.cuda_graph_config.cuda_graph_max_batch_size != 0:
                    if self.cuda_graph_config.cuda_graph_batch_sizes != self._generate_cuda_graph_batch_sizes(
                            self.cuda_graph_config.cuda_graph_max_batch_size,
                            self.cuda_graph_config.cuda_graph_padding_enabled):
                        raise ValueError(
                    ...
                    self.cuda_graph_config.cuda_graph_max_batch_size = max(
                        self.cuda_graph_config.cuda_graph_batch_sizes)
            else:
                max_batch_size = self.cuda_graph_config.cuda_graph_max_batch_size or 128
                generated_sizes = self._generate_cuda_graph_batch_sizes(
                    max_batch_size,
                    self.cuda_graph_config.cuda_graph_padding_enabled)
                self.cuda_graph_config.cuda_graph_batch_sizes = generated_sizes
                self.cuda_graph_config.cuda_graph_max_batch_size = max_batch_size
