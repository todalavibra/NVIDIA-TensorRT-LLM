...
void attention_inplace(torch::Tensor q, torch::optional<torch::Tensor> k, torch::optional<torch::Tensor> v,
    ...
    std::optional<int64_t> attention_chunk_size, bool const use_spec_dec,
    std::optional<torch::Tensor> spec_decoding_position_offsets, std::optional<torch::Tensor> spec_decoding_packed_mask,
    std::optional<torch::Tensor> spec_decoding_generation_lengths)
{
    ...
    if ((num_contexts > 0) && (attn_input_type != AttentionInputType::GenerationOnly))
    {
        ...
        runner->run(*op,
            ...
            mla_context_kv_cache_block_offsets, spec_decoding_position_offsets, spec_decoding_packed_mask,
