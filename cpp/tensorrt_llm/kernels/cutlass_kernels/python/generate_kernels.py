...
def GetDataTypeNames(type, is_mx_fp4=None):
    mxprefix = ""
    if is_mx_fp4 is not None:
        mxprefix = "mx_" if is_mx_fp4 else "nv_"
    if isinstance(type, e2m1_type):
...
class TrtLlm_GemmLauncher:
    ...
    def __init__(self,
        ...
        self.is_mx_fp4 = is_mx_fp4

    def __repr__(self):
        kernel_prefix = "{}_sm{}_{}_{}_{}_{}_{}_{}_{}_{}x{}x{}_{}x{}x{}_{}".format(
            GemmKindNames[self.gemm_kind], self.arch,
            GetDataTypeNames(self.act_type, self.is_mx_fp4),
            GetDataTypeNames(self.weight_type, self.is_mx_fp4),
...
"""
        ...
        INSTANTIATE_TMA_WARP_SPECIALIZED_MOE_GEMM({arch_tag}, {act_tag}, {weight_tag}, {out_tag}, {epi_tag}, {epi_fusion}, {operation.cta_shape[0]}, {operation.cta_shape[1]}, {operation.cta_shape[2]}, {operation.cga_shape[0]}, {operation.cga_shape[1]}, {operation.cga_shape[2]}, {"true" if operation.is_mx_fp4 else "false"}, false);
#endif"""
...
def generate_sm120_grouped_gemm_operations(is_arch_enabled):
    ...
    quant_ops = [TrtLlm_QuantOp.none]
    epi_tags = [TrtLlm_EpilogueTag.epilogue_op_default]
    cta_shapes_mnk = [[128, 128, 128], [128, 128, 256], [256, 128, 128], [128, 256, 128]]
    ...
    for dtype, quant_op, epi_tag, epi_fusion, cta_shape_mnk, cga_shape in partial_args:
        ...
        for otype in otypes:
            moe_gemm_operation = TrtLlm_GemmLauncher(
                GemmKind.Grouped, arch, dtype, dtype, dtype, dtype, otype,
                quant_op, epi_tag, cta_shape_mnk, warp_shape, stages,
    ...
    for dtype, quant_op, epi_tag, epi_fusion, cta_shape_mn, cga_shape in partial_args:
        ...
        for otype in otypes:
            moe_gemm_operation = TrtLlm_GemmLauncher(
                ...
                epi_schedule,
                epi_fusion,
                is_mx_fp4=(dtype == DataType.e4m3 and weight_type == e2m1))
